...
CHEF_SERVER_NAME="Chef-Automate-us-east-1" # The name of your Chef Server
CHEF_SERVER_ENDPOINT="chef-automate-us-east-1-ncmov8e4otrxfd3l.us-east-1.opsworks-cm.io" # The FQDN of your Chef Server
...
RUN_LIST="role[opsworks-example-role]" # Use "role[opsworks-example-role]" when following the starter kit example or specify recipes like recipe[chef-client],recipe[apache2] etc.
...
prepare_os_packages() {
  ...
  if [[ ${OS} = *"Ubuntu"* ]]; then
    ...
    apt-get -y install unzip python python-pip
